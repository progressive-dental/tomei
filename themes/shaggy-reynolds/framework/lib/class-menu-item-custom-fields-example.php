...
class Menu_Item_Custom_Fields_Example {
  ...
  public static function init() {
    ...
    self::$fields = array(
      'icon' => __( 'Custom icon class', 'menu-item-custom-icon-class' ),
      'make_button' => __( 'Make link a button', 'menu-item-custom-icon-class' )
    );
  }
  ...
  public static function _fields( $id, $item, $depth, $args ) {
    foreach ( self::$fields as $_key => $label ) :
      ...
      <?php
    endforeach;
    $name  = sprintf( 'menu-item-%s[%s]', 'make_button', $item->ID );
    $id    = sprintf( 'edit-%s-%s', 'make_button', $item->ID );
    ?>
      <div class="menu-settings-input checkbox-input">
        <input type="checkbox" id="<?php echo $id; ?>" name="<?php echo $name; ?>" class="widefat  make_button" value="yes" <?php if(get_post_meta( $item->ID, 'menu-item-make_button', true) == 'yes'): ?>checked="checked"<?php endif; ?> />
        <label for="make_button">Make link a button</label>
      </div>
    <?php
  }
