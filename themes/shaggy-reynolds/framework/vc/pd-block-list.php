...
if ( ! class_exists( 'PdBlockList' ) ) {
  ...
  class PdBlockList {
    ...
    function __construct() {
      ...
      add_filter( 'init', array( $this, 'create_bl_shortcodes' ), 999 );

      // Render shortcode for the plugin.
      add_shortcode( 'pd_block_list', array( $this, 'render_bl_shortcodes' ) );
    }
     ...
     * @since 1.0
     */
    public function create_bl_shortcodes() {
      if ( ! function_exists( 'vc_map' ) ) {
        return;
      }

      // $default_content = '[vc_row_inner][vc_column_inner width="1/1"][/vc_column_inner][/vc_row_inner]';
      $default_content = '[pd_block][/pd_block][pd_block][/pd_block][pd_block][/pd_block]';
      // $default_content = '';
      ...
      block_list_row_fixes();

      vc_map( array(
        'name' => __( 'Block List', 'progressive' ),
        'base' => 'pd_block_list',
        'icon' => THEME_WEB_ROOT . '/assets/images/pd-logo.png',
        'description' => __( 'A container for the block list.', 'progressive' ),
     ...
     * @since 1.0
     */
    public function render_bl_shortcodes( $atts, $content = null ) {
...
function block_list_row_fixes() {
   ...
   */
  if ( class_exists( 'WPBakeryShortCodesContainer' ) && ! class_exists( 'WPBakeryShortCode_Pd_Block_list' ) ) {
    $create_class = true;
  } else {
  ...
  if ( $create_class ) {
     ...
     */
    class WPBakeryShortCode_Pd_Block_list extends WPBakeryShortCodesContainer {
    }
  }
}
