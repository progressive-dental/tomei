...
if ( ! class_exists( 'GambitCarouselAnything2' ) ) {

  /**
   * Sorry, but that doesn't include your kitchen sink.
   */
  class GambitCarouselAnything2 {
    ...
    function __construct() {
      ...
      add_shortcode( 'pd_sponsors_list', array( $this, 'render_ca_shortcodes' ) );
    }
    ...
    public function create_ca_shortcodes() {
      ...
      $default_content = '[pd_sponsor size="medium"][/pd_sponsor][pd_sponsor size="medium"][/pd_sponsor][pd_sponsor size="medium"][/pd_sponsor][pd_sponsor size="medium"][/pd_sponsor]';
      // $default_content = '';
      ...
      vc_map( array(
        'name' => __( 'Sponsors List', 'progressive' ),
        'base' => 'pd_sponsors_list',
        'icon' => THEME_WEB_ROOT . '/assets/images/pd-logo.png',
        'description' => __( 'A container for the sponsors.', 'progressive' ),
    ...
    public function render_ca_shortcodes( $atts, $content = null ) {
      ...
      $ret = '';
      // Carousel html.
      $ret .= '<ul class="sponsor-list">';
      $ret .= do_shortcode( $content ) . '</ul>';

      return $ret;
    }
  }
  new GambitCarouselAnything2();
}
...
function sponsor_row_fixes() {
   ...
   */
  if ( class_exists( 'WPBakeryShortCodesContainer' ) && ! class_exists( 'WPBakeryShortCode_Pd_Sponsors_list' ) ) {
    $create_class = true;
  } else {
  ...
  if ( $create_class ) {
     ...
     */
    class WPBakeryShortCode_Pd_Sponsors_list extends WPBakeryShortCodesContainer {
    }
  }
}
